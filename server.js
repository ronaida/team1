...
var mainInstructorHtml = fs.readFileSync(path.join(__dirname, 'static/Instructorpage.html'),'utf8');
...
app.post('/api/student_update', auth.ensureApiAuth, (req, res) => {
  ...
  if(studentUpdates.solution_disabled =='disabled' ){
    studentUpdates.solution_disabled='0';
  }
  if(studentUpdates.solution_disabled =='enabled')
  {
...
app.get('/api/students', async (req, res) => {
  ...
  db.fetchMystudents(null,instructor_id,null,async function(studentsList){
    ...
    for (let i = 0; i < studentsList.length; i++) {
      ...
      {
        studentsList[i]["solution_disabled"]="disabled";
      }
...
//get instructoractivity
app.get('/api/instructoractivity',  (req, res) => {
  var query = req.query.query;
  if(util.isNullOrUndefined(query)) query = "";
  query = query.trim();
  if(query !== "" && !validator.matches(query,/^[A-Z'\-\s]+$/i)){
  ...
  db.fetchInstructorActivity(query,100,null,function(activityList){
    returnListWithChallengeNames(res,activityList);
  });
});
//get the students levels
app.get('/api/students',  (req, res) => {
  ...
  console.log(req.params);
    ...
    let userLevelForModule =  challenges.getUserLevelForModule(req.user, moduleId);
  
    //res.send({"level":userLevelForModule});
    returnListWithChallengeNames(res,userLevelForModule);
});
